...
///当前的VC
@MainActor public func JQ_currentViewController() -> UIViewController {
...
///当前的NavVC
@MainActor public func JQ_currentNavigationController() -> UINavigationController {
...
/// 动态加载VC
@MainActor public func JQ_loadVC(string:String)->UIViewController?{
    //动态获得命名空间
...
//                JQ_ShowError(errorStr: error?.localizedDescription ?? "")
    ...
    ///   - cancelTitle: 取消
    ///   - clouse: 回调结果
    public static func touchIDAuth(_ cancelTitle:String = "取消认证",clouse:@escaping @Sendable (JQTouchAuthType,NSError?)->Void){
...
//        let soundThere = SystemSoundID(1521)
    ...
    ///   - clouse: 返回数据，是否有更新
    public static func checkVersion(appid:String,_ clouse:(@Sendable (Bool,VersionResultModel?,URL?)->Void)? = nil){
...
public class VersionModel:Codable{
    public var resultCount = 1
    public var results = [VersionResultModel]()
}

public class VersionResultModel:Codable{
