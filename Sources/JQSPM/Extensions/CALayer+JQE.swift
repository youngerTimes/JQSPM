...
#if canImport(UIKit)
import UIKit
extension CALayer:JQFisherCompatible{}

extension JQFisher where Base:CALayer{
    var identity:String{
    ...
    var cornerRadius:CGFloat{
        get{return self.base.cornerRadius}
        set(value){self.base.cornerRadius = value}
    }

    var masksToBounds:Bool{
        get{return self.base.masksToBounds}
        set(value){self.base.masksToBounds = value}
    }

    var borderWidth:CGFloat{
        get{return self.base.borderWidth}
        set(value){self.base.borderWidth = value}
    }

    var borderCololr:CGColor?{
        get{return self.base.borderColor}
        set(value){self.base.borderColor = value}
    }

    /// size
    var size: CGSize {
        get {return self.base.frame.size}
        set {self.base.frame.size = newValue}
    }
}
...
extension JQFisher where Base:CALayer{
    ...
    func cornerPart(byRoundingCorners corners: UIRectCorner, radii: CGFloat) {
...
extension JQFisher where Base:CALayer{
    ...
    ///   - removedOnCompletion: 运动后的位置保持不变（layer的最后位置是toValue）
    ///   - option: 动画的时间节奏控制 方式
    func addKeyframeAnimationRotation(values: [Any] = [CGFloat(-5).jq.degrees, CGFloat(5).jq.degrees, CGFloat(-5).jq.degrees],
                                         keyTimes: [NSNumber]?,
