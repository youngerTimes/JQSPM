...
extension String {
    var hexColor: CustomColor {
        ...
        return CustomColor(red: red, green: green, blue: blue, alpha: 1)
    }
}
...
public extension String{
    ...
    // MARK: -- Static
    /// 获取字符串宽度
    static func getWidth(text: String, height: CGFloat, font: CGFloat) -> CGFloat {
        let text = text as NSString
        let rect = text.boundingRect(with: CGSize(width: CGFloat(Int.max), height: height), options: .usesLineFragmentOrigin, attributes: [.font : CustomFont.systemFont(ofSize: font)], context: nil)
        return rect.size.width
    }

    /// 从文本中提取所有链接
    /// - Returns: 提取的url
    func pickUpUrls() -> [String] {
        var urls = [String]()
        // 创建一个正则表达式对象
        do {
    ...
    ///获取字符串高度
    static func getHeight(text: String, width: CGFloat, font: CGFloat) -> CGFloat {
        let text = text as NSString
        let rect = text.boundingRect(with: CGSize(width: width, height: CGFloat(Int.max)), options: .usesLineFragmentOrigin, attributes: [.font : CustomFont.systemFont(ofSize: font)], context: nil)
        return rect.size.height
    }


    ///获取字符串高度(带font)
//    static func getHeightwithFont(text: String, width: CGFloat, font: CustomFont) -> CGFloat {
//        let text = text as NSString
...
//    }
    ...
    func mutableBytes(_ clouse:((UnsafeMutablePointer<UInt8>)->Void)?){
    ...
    func subString(sub:String)->(index:NSInteger,length:NSInteger){
    ...
    @available(*,deprecated,message: "废弃:建议使用subRange")
    ...
    /// 获取子字符串的NSRange
    func subRange(_ subText:String)->NSRange?{
...
//    }
    ...
    ///JSONString转换为数组
    static func toArray(jsonString:String) ->NSArray{
    ...
    ///字符串分割成数组
    func toArray(character:String) -> Array<String>{
    ...
    func filterDecimals(replacementString:String,range:NSRange,limit:NSInteger = 2)->Bool{
    ...
    ///适配Web,填充HTML的完整
//    func wrapHtml(edge:CustomEdgeInsets)-> String{
...
"""
    ...
    /// - Returns: 返回富文本
    func htmlToAttibute(lossy:Bool = false)->NSMutableAttributedString?{
    ...
    ///JsonString->字典
    func dictionaryFromJsonString() -> Any? {
    ...
    /// 将HTML标签中<>去除
    @available(*,deprecated,message: "废弃")
    static func filterFromHTML(_ htmlString:String)->String{
        var html = htmlString
    ...
    ///转化为Base64
    func safeBase64(type: JQSafeBase64Type) -> String {
    ...
    func centeredText(with centerSubstring: String, rangeBeforeAfter: Int) -> String {
    ...
    func idCardBirthday(formatter:String = "")->String?{
        ...
        for line in lines {
            if line.hasPrefix("#") {
                ...
                if let section = currentSection {
                    ...
                    updatedSection.content = currentContent.trimmingCharacters(in: .whitespacesAndNewlines)
