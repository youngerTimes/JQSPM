...
public extension JQFisher where Base:UIImage{
    ///压缩图片，限制最大宽高
    func resizeImage(maxWidth: CGFloat = 1280,maxHeight: CGFloat = 720) -> UIImage{
        ...
        self.base.draw(in: CGRect(x:0, y:0, width:sizeChange.width, height:sizeChange.height))
    ...
    /// 生成一张高斯模糊图
    func blur(_ value:CGFloat)->UIImage{
...
public extension UIImage{
    ...
    /// - Returns: 返回图片
    @MainActor func jq_scaleImage(scaleSize:CGFloat)->UIImage {
    ...
    /// - Returns: 返回图片
    @MainActor private func jq_reSizeImage(reSize:CGSize)->UIImage {
    ...
    ///   - size: 图片尺寸
    /// - Returns: 返回对应的图片
    @MainActor static func jq_image(color: UIColor, size: CGSize = CGSize(width: 1.0, height: 1.0)) -> UIImage? {
    ...
    ///   - round: 圆角大小
    /// - Returns: 返回对应的图片
    @MainActor static func jq_image(color: UIColor, size: CGSize, corners: UIRectCorner, radius: CGFloat) -> UIImage? {
    ...
    ///   - direction: 渐变的方向
    /// - Returns: 渐变的图片
    @MainActor static func jq_gradient(_ hexsString: [String], size: CGSize = CGSize(width: 1, height: 1), locations:[CGFloat]? = nil, direction: GradientDirection = .horizontal) -> UIImage? {
    ...
    ///   - direction: 渐变的方向
    /// - Returns: 渐变的图片
    @MainActor static func jq_gradient(_ colors: [UIColor], size: CGSize = CGSize(width: 10, height: 10), locations:[CGFloat]? = nil, direction: GradientDirection = .horizontal) -> UIImage? {
...
public extension UIImage {
    ...
    static var shared: NSCache<AnyObject, AnyObject>! {
        struct StaticSharedCache {
            nonisolated(unsafe) static let shared: NSCache<AnyObject, AnyObject>? = NSCache()
        }
    ...
    /**
     ...
     - Returns A new image
     */
    @MainActor convenience init?(text: String, font: UIFont = UIFont.systemFont(ofSize: 18), color: UIColor = UIColor.white, backgroundColor: UIColor = UIColor.gray, size: CGSize = CGSize(width: 100, height: 100), offset: CGPoint = CGPoint(x: 0, y: 0)) {
        let label = UIKit.UILabel(frame: CGRect(x: 0, y: 0, width: size.width, height: size.height))
        label.font = font
        label.text = text
    ...
    /**
     ...
     - Returns A new image
     */
    @MainActor convenience init?(fromView view: UIKit.UIView) {
     ...
     */
    @MainActor func applyLightEffect() -> UIImage? {
    ...
    /**
     ...
     - Returns New image or nil
     */
    @MainActor func applyExtraLightEffect() -> UIImage? {
    ...
    /**
     ...
     - Returns New image or nil
     */
    @MainActor func applyDarkEffect() -> UIImage? {
    ...
    /**
     ...
     - Returns New image or nil
     */
    @MainActor func applyTintEffect(tintColor: UIColor) -> UIImage? {
    ...
    /**
     ...
     - Returns New image or nil
     */
    @MainActor func applyBlur(withRadius blurRadius: CGFloat, tintColor: UIColor?, saturationDeltaFactor: CGFloat, maskImage: UIImage? = nil) -> UIImage? {
    ...
    /**
     ...
     - Returns A new image
     */
    class func image(fromURL url: String, placeholder: UIImage, shouldCacheImage: Bool = true, closure:  @Sendable @escaping (_ image: UIImage?) -> ()) -> UIImage? {
        // From Cache
        if shouldCacheImage {
...
public extension UIImage {
    ...
    /// - Parameter mode: 压缩模式
    /// - Returns: 压缩后Data
    @MainActor func compress(mode: CompressionMode = .medium) -> Data? {
    ...
    func asyncCompress(mode: CompressionMode = .medium, queue: DispatchQueue = DispatchQueue.global(), complete:@Sendable @escaping (Data?, CGSize) -> Void) {
        queue.async {
