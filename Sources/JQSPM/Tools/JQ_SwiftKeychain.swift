...
open class SwiftKeychain: NSObject {
    ...
    /// Instantiates an `SwiftKeychain` object.
    public override init() {
        super.init()
    }

    /// Instantiates a SwiftKeychain object.
    ///
    ...
    /// Creates an instance of SwiftKeychain with the class method.
    ///
    /// - Returns: An instance of SwiftKeychain.
    @objc public class func createKeychain() -> SwiftKeychain {
        return SwiftKeychain()
    }

    /// Returns a keychain that copies the current SwiftKeychain instance.
    ///
    /// - Returns: A SwiftKeychain object.
    @objc open override func copy() -> Any {
        let keychain = SwiftKeychain.createKeychain()
    ...
    @objc public func add(_ value: String?, forKey key: String) -> Bool {
        let opts = SwiftKeychainAccessOptions.accessibleWhenUnlocked
    ...
    ///   - value: The text value to be written to the keychain.
    ///   - key: The key which the text is stored in the keychain.
    ///   - options: The options indicates when you app needs access to the text in the keychain. By the default SwiftKeychainAccessOptions.accessibleWhenUnlocked option is used that permits the data to be accessed only while the device is unlocked by the user.
    /// - Returns: True if the text was successfully written to the keychain, false otherwise.
    @discardableResult
    @objc public func add(_ value: String?, forKey key: String, options: SwiftKeychainAccessOptions) -> Bool {
    ...
    ///   - value: The text value to be written to the keychain.
    ///   - key: The key which the text is stored in the keychain.
    ///   - access: The parameter indicates when you app needs access to the text in the keychain. By the default SwiftKeychain.AccessOptions.accessibleWhenUnlocked option is used that permits the data to be accessed only while the device is unlocked by the user.
    /// - Returns: True if the text was successfully written to the keychain, false otherwise.
    @discardableResult
    public func set(_ value: String?, forKey key: String, withAccess access: SwiftKeychain.AccessOptions? = nil) -> Bool {
        let v = value?.data(using: String.Encoding.utf8)
    ...
    /// - Returns: True if the data was successfully written to the keychain, false otherwise.
    @discardableResult
    @objc public func addData(_ value: Data?, forKey key: String, options: SwiftKeychainAccessOptions) -> Bool {
        let opts = toOpts(options)
    ...
    /// - Returns: True if the data was successfully written to the keychain, false otherwise.
    @discardableResult
    public func set(_ value: Data?, forKey key: String, withAccess access: SwiftKeychain.AccessOptions? = nil) -> Bool {
        // The lock prevents the code to be run simultaneously from multiple threads which may result in crashing.
        lock.lock()
        defer { lock.unlock() }

        let accessible = access?.value ?? SwiftKeychain.AccessOptions.defaultOption.value

        var query: [String: Any] = supplyQueryDictionary()
        query[SwiftKeychain.Constants.account] = key
        query[SwiftKeychain.Constants.accessible] = accessible
        ...
        guard status == errSecSuccess else {
            if let v = value {
                query[SwiftKeychain.Constants.valueData] = v
                queryDictionary?[SwiftKeychain.Constants.valueData] = v
        ...
        if let v = value {
            let updatedDictionary: [String: Any] = [
                SwiftKeychain.Constants.valueData: v
            ]
    ...
    /// - Returns: True if the boolean value was successfully written to the keychain, false otherwise.
    @discardableResult
    @objc public func addBool(_ value: Bool, forKey key: String, options: SwiftKeychainAccessOptions) -> Bool {
        let opts = toOpts(options)
        return set(value, forKey: key, withAccess: opts)
    }
    ...
    /// - Returns: True if the boolean value was successfully written to the keychain, false otherwise.
    @discardableResult
    public func set(_ value: Bool, forKey key: String, withAccess access: SwiftKeychain.AccessOptions? = nil) -> Bool {
        let v = value ? "1" : "0"
    ...
    @objc public func getData(_ key: String, asReference: Bool = false) -> Data? {
        ...
        var query: [String: Any] = supplyQueryDictionary()
        query[SwiftKeychain.Constants.account] = key
        query[SwiftKeychain.Constants.matchLimit] = kSecMatchLimitOne

        if asReference {
            query[SwiftKeychain.Constants.returnReference] = kCFBooleanTrue
        } else {
            query[SwiftKeychain.Constants.returnData] = kCFBooleanTrue
        }
        queryDictionary = query
    ...
    private func supplyQueryDictionary(shouldAddItem: Bool = false) -> [String: Any] {
        var query: [String: Any] = [
            SwiftKeychain.Constants.kClass: kSecClassGenericPassword
        ]

        if let accessGroup = accessGroup {
            query[SwiftKeychain.Constants.accessGroup] = accessGroup
        }

        if synchronizable {
            let key = SwiftKeychain.Constants.synchronizable
            query[key] = shouldAddItem ? kCFBooleanTrue : kSecAttrSynchronizableAny
        }

        if let serviceId = serviceIdentifier {
            query[SwiftKeychain.Constants.service] = serviceId
        }
...
// MARK: - Extension SwiftKeychain
extension SwiftKeychain {
    ...
    /// Converts a SwiftKeychainAccessOptions value to a SwiftKeychain.AccessOptions value.
    ///
    /// - Parameter opts: A SwiftKeychainAccessOptions value.
    /// - Returns: A SwiftKeychain.AccessOptions value.
    private func toOpts(_ opts: SwiftKeychainAccessOptions) -> SwiftKeychain.AccessOptions? {
        var options: SwiftKeychain.AccessOptions? = nil

        switch opts {
            case .accessibleWhenUnlocked:
                options = SwiftKeychain.AccessOptions.accessibleWhenUnlocked
                break
            case .accessibleWhenUnlockedThisDeviceOnly:
                options = SwiftKeychain.AccessOptions.accessibleWhenUnlockedThisDeviceOnly
                break
            case .accessibleAfterFirstUnlock:
                options = SwiftKeychain.AccessOptions.accessibleAfterFirstUnlock
                break
            case .accessibleAfterFirstUnlockThisDeviceOnly:
                options = SwiftKeychain.AccessOptions.accessibleAfterFirstUnlockThisDeviceOnly
                break
            case .acessibleWhenPasscodeSetThisDeviceOnly:
                options = SwiftKeychain.AccessOptions.acessibleWhenPasscodeSetThisDeviceOnly
                break
            case .accessibleAlways:
                options = SwiftKeychain.AccessOptions.accessibleAlways
                break
            case .accessibleAlwaysThisDeviceOnly:
                options = SwiftKeychain.AccessOptions.accessibleAlwaysThisDeviceOnly
                break
            default: break
        }
...
@objc public enum SwiftKeychainAccessOptions: UInt8 {
