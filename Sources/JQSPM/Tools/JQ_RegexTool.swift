...
public struct JQ_RegexTool {
    ...
    //使用正则表达式进行初始化
    @MainActor public init(_ pattern: RegexPattern, options: Options = []) throws {
...
extension JQ_RegexTool {
    ...
    public struct Options: OptionSet {
        ...
        //忽略字母
        @MainActor public static let ignoreCase = Options(rawValue: 1)

        //忽略元字符
        @MainActor public static let ignoreMetacharacters = Options(rawValue: 1 << 1)

        //默认情况下,“^”匹配字符串的开始和结束的“$”匹配字符串,无视任何换行。
        //使用这个配置，“^”将匹配的每一行的开始,和“$”将匹配的每一行的结束。
        @MainActor public static let anchorsMatchLines = Options(rawValue: 1 << 2)

        ///默认情况下,"."匹配除换行符(\n)之外的所有字符。使用这个配置，选项将允许“.”匹配换行符
        @MainActor public static let dotMatchesLineSeparators = Options(rawValue: 1 << 3)

        //OptionSet的 raw value
        public let rawValue: Int

        //将Regex.Options 转换成对应的 NSRegularExpression.Options
        @MainActor var toNSRegularExpressionOptions: NSRegularExpression.Options {
