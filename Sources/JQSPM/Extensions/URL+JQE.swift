...
#if canImport(UIKit)
extension URL:JQFisherCompatible{}

extension JQFisher where Base == URL{

    ///根据后缀获取对应的Mime-Type
    var mimeType:String{
        get{
            if let uti = UTTypeCreatePreferredIdentifierForTag(kUTTagClassFilenameExtension,base.pathExtension as NSString,nil)?.takeRetainedValue() {
...
extension URL{
    ...
    static func splitVideoFileUrlFps(splitFileUrl: URL, fps: Float, splitCompleteClosure:@Sendable @escaping (Bool, [UIImage]) -> Void) {
