//
//  File.swift
//  JQSPM
//
//  Created by 无故事王国 on 2024/10/17.
//

import WebKit

#if canImport(UIKit)
extension WKWebView:JQFisherCompatible{}

extension JQFisher where Base:WKWebView{
    ///获取HTML中的所有图片，写在 didFinishNavigation 【部分一】
    /**
     func webView(_ webView: WKWebView, decidePolicyFor navigationAction: WKNavigationAction, decisionHandler: @escaping (WKNavigationActionPolicy) -> Void) {
     let url  = navigationAction.request.url
     if url?.scheme == "image-preview-index"{
     let browser = SDPhotoBrowser()
     browser.imageCount = htmlImgsUrl!.count
     let last = url?.absoluteString.split(separator: ":").last
     let index = Int("\(last!)") ?? 0
     print(index)
     browser.currentImageIndex = index
     browser.delegate = self
     browser.show()
     }
     decisionHandler(WKNavigationActionPolicy.allow)
     }

     */
    @MainActor func htmlImgTag(resultForImg:@escaping ([String],[UIImageView])->(Void)){

        var imgsUrl:Array<String> = []
        var imgs:Array<UIImageView> = []

        //获取所有图片的标签
        let getAllImgsTag = """
            function getImages(){\
                var imgs = document.getElementsByTagName('img');\
                 var imgScr = '';\
                 for(var i=0;i<imgs.length;i++){\
                 if (i == 0){ \
                 imgScr = imgs[i].src; \
                 } else {\
                 imgScr = imgScr +'***'+ imgs[i].src;\
                 } \
                 };\
                 return imgScr;\
                 };
            """
        self.base.evaluateJavaScript(getAllImgsTag, completionHandler: nil)

        self.base.evaluateJavaScript("getImages()") { @Sendable (result, error) in
            if((error) != nil){return}
            if (result != nil){
                let res = result as! String
                imgsUrl = Array(res.components(separatedBy: "***"))
                for img in imgsUrl{
                    let imageView = UIImageView()

                    if let url = URL(string: img){
                        let req = URLRequest(url: url)
                         URLSession.shared.dataTask(with: req) { data, resp, error in
                            guard error != nil else {return}
                             DispatchQueue.main.async {
                                 imageView.image = UIImage(data: data!)
                             }
                        }
                    }
                    imgs.append(imageView)
                }
                resultForImg(imgsUrl,imgs)
            }
        }

        let imgClick = """
         function registerImageClickAction(){\
         var imgs = document.getElementsByTagName('img');\
         for(var i=0;i<imgs.length;i++){\
         imgs[i].customIndex = i;\
         imgs[i].onclick=function(){\
         window.location.href='image-preview-index:'+this.customIndex;\
            }\
         }\
         }
"""
        self.base.evaluateJavaScript(imgClick, completionHandler: nil)
        self.base.evaluateJavaScript("registerImageClickAction()", completionHandler: nil)
    }
}
#endif
