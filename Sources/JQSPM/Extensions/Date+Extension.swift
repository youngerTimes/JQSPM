...
extension Date:JQFisherCompatible{}
...
public extension Date{
    ...
    /// 当月第一天是星期几
    static func weekDayInCurrentMonth() ->Int {
        //星期和数字一一对应 星期日：7
        let dateFormatter = DateFormatter()
    ...
    /// 计算两个时间段的间隔
    static func CalBySet(startDate:Date,endDate:Date,set:Set<Calendar.Component>? = Set(arrayLiteral: Calendar.Component.day,Calendar.Component.year,Calendar.Component.month,Calendar.Component.hour,Calendar.Component.minute))->(year:Int,month:Int,day:Int,hour:Int,minute:Int,second:Int){
        //格式化成：秒归零
        let formatStartDate = Date.format(year: startDate.year, month: startDate.month, day: startDate.day,hour:startDate.hour,minute: startDate.minute,second:startDate.second)
        let formatEndDate = Date.format(year: endDate.year, month: endDate.month, day: endDate.day,hour:endDate.hour,minute: endDate.minute,second:endDate.second)
    ...
    /// 计算两个时间段的间隔
    /// - Returns: 返回天数
    static func CalByDays(startDate:Date,endDate:Date)->Int{
        //格式化成：时分秒归零
        let formatStartDate = Date.format(year: startDate.year, month: startDate.month, day: startDate.day,hour:0,minute: 0)
        let formatEndDate = Date.format(year: endDate.year, month: endDate.month, day: endDate.day,hour:0,minute: 0)
    ...
    /// 计算两个时间段的间隔【完成了格式化操作】
    static func CalByStr(startDate:Date,endDate:Date)->String{
        var str = ""
        let calendar = Calendar(identifier: .gregorian)
    ...
    static func CalByEnum(startDate:Date,endDate:Date)->(year:Int,month:Int,day:Int,hour:Int,minute:Int){
        let calendar = Calendar.current
        let set = Set(arrayLiteral: Calendar.Component.day,Calendar.Component.year,Calendar.Component.month,Calendar.Component.hour,Calendar.Component.minute)
        let a = calendar.dateComponents(set, from: startDate, to: endDate)
    ...
    static func CalByEnumDay(startDate:Date,endDate:Date)->(day:Int,hour:Int){
        let calendar = Calendar.current
    ...
    ///时间戳转换字符串，yyyy-MM-dd
    static func TheTimeStampConversionMinutes(_ time:TimeInterval, _ formatter:String) -> String {
    ...
    ///时间字符串转时间戳
    static func StringToTimeInterval(_ dateStr:String, _ formatter:String) -> TimeInterval {
        let dateFormatter = DateFormatter()
        dateFormatter.dateFormat = formatter
    ...
    static func TimeBeforeInfoToString(_ timeIntrval:TimeInterval) -> String {
        let dateFormatter = DateFormatter()
        ...
        if year > 0 {
            return self.TheTimeStampConversionMinutes(timeIntrval, "yyyy-MM-dd")
        }else if month > 0 {
            return self.TheTimeStampConversionMinutes(timeIntrval, "MM-dd")
        }else if day > 0 {
            if day > 7 {
                return "一周前"
            }else {
            ...
            if isToday(timeterval: timeIntrval) {
                return self.TheTimeStampConversionMinutes(timeIntrval, "HH:mm")
            }else {
                return self.TheTimeStampConversionMinutes(timeIntrval, "昨天 HH:mm")
            }
        }else if minute > 0 {
            return "\(minute)分钟前"
        }else {
    ...
    ///将时间戳传入，返回距离今天有多少小时
    static func timeBeforHours(_ timeIntrval:TimeInterval) -> Int {
        let dateFormatter = DateFormatter()
        dateFormatter.dateFormat = "yyyy-MM-dd HH:mm:ss"
        let nowTime = NSDate().timeIntervalSince1970
    ...
    ///判断一个时间戳是否为今天
    static func isToday(timeterval:TimeInterval) -> Bool {
        return Calendar.current.isDateInToday(Date(timeIntervalSince1970: timeterval))
    }

    /// 判断一个时间戳是否为昨天
    static func isYesterday(timeterval:TimeInterval) -> Bool {
    ...
    /// - Parameter year: 设定年
    /// - Parameter month: 设定月
    static func getDays(_ year:Int,_ month:Int)->Int{
    ...
    /// 给定年月日，得到Date的时间类型
    static func format(year:NSInteger,month:NSInteger,day:NSInteger,hour:NSInteger = 0,minute:NSInteger = 0,second:NSInteger = 0)->Date?{
        let calendar = Calendar(identifier: .gregorian)
    ...
    /// 根据时间戳，返回年龄
    static func calculateAge(time: TimeInterval) -> NSInteger {
    ...
    ///获取上一个周
    static func lastWeek(_ date:Date)->Date{
        return date.addingTimeInterval(-7 * 24 * 3600)
    }

    ///获取下一个周
    static func nextWeek(_ date:Date)->Date{
        return date.addingTimeInterval(7 * 24 * 3600)
    }

    ///根据时间获取一个周的时间表
    static func weekDates(_ date:Date)->[Date]?{
        //当前时间
    ...
    /// - Returns: 返回元祖
    static func secondToDate(interval:Int)->(hour:Int,minute:Int,second:Int){
    ...
    ///时间戳格式格式化时间
    static func format(_ time:TimeInterval,formatter:String)->String{
        let detaildate = NSDate(timeIntervalSince1970: time)
        let dateFormatter = DateFormatter()
        dateFormatter.dateFormat = formatter
        return dateFormatter.string(from: detaildate as Date)
    ...
    ///时间戳格式格式化时间
    static func formatNow(formatter:String)->String{
        let detaildate = Date()
        let dateFormatter = DateFormatter()
        dateFormatter.dateFormat = formatter
        return dateFormatter.string(from: detaildate as Date)
    ...
    ///时间戳格式格式化时间
    static func formatDate(_ time:TimeInterval,formatter:String)->Date{
    ...
    static func formateToTime(_ time:Int)->String{
    ...
    /// 将秒转换为00:00格式，仅限于 24 * 3600 内
    static func formateToTime_tuple(_ time:Int)->(hour:Int,minute:Int,second:Int){
    ...
    /// 判断两个时间是否相等，根据格式判断
    static func absEqual(date1:Date,date2:Date,format:String = "yyyy-MM-dd") -> Bool{
        return date2.jq.format(format) == date1.jq.format(format)
    }
}

public extension Date{}

public extension JQFisher where Base == Date{
    // MARK: -- Property
    ///该时间所在周的第一天日期（2017年12月17日 00:00:00）
    var startOfWeek: Date {
        let calendar = NSCalendar.current
        let components = calendar.dateComponents(
            Set<Calendar.Component>([.yearForWeekOfYear, .weekOfYear]), from: self.base)
    ...
    var lastWeekDates:[Date]{
        let temp_lastDate = self.base.addingTimeInterval(-7 * 24 * 3600)
        return Date.weekDates(temp_lastDate)!
    }

    ///获取上一周的时间表
    var nextWeekDates:[Date]{
        let temp_nextDate = self.base.addingTimeInterval(7 * 24 * 3600)
        return Date.weekDates(temp_nextDate)!
    }

    ///获取本周的时间表
    var currentWeekDates:[Date]{
        return Date.weekDates(self.base)!
    }

    ///获取上一个周
    var lastWeekDate:Date{
        let temp = self.base.addingTimeInterval(TimeInterval(TimeZone.current.secondsFromGMT()))
    ...
    func isToday()->Bool {
        let calendar = NSCalendar.current
        let com = calendar.dateComponents([.year,.month,.day], from: self.base)
...
public extension JQFisher where Base == Date{
    ...
    ///将秒数转换为时分秒的字符串
    @available(*,deprecated,message: "废弃:使用 secondToDate")
    func transSingleToHourMinSec(time: Float,type:JQDateType = .HMS) -> String{
    ...
    func add(year:Int? = nil,month:Int? = nil,day:Int? = nil,hour:Int? = nil,minute:Int? = nil,second:Int? = nil)->Date?{
        let calendar = Calendar(identifier: .gregorian)
        return calendar.date(byAdding: DateComponents(calendar: calendar,year: year,month: month,day: day,hour: hour,minute: minute,second: second), to: self.base)


    }

    func clearTimes()->Date!{
        let calendar = Calendar(identifier: .gregorian)
        let dateComponents = DateComponents(calendar: calendar, timeZone: TimeZone.current, year: base.year, month: base.month, day: base.day, hour: base.hour, minute: 0, second: 0, nanosecond: 0)
        return dateComponents.date
    }

    /// 获取指定年份
    func allYears(_ startYear:Int = 1900)->Array<Int>{
        var years = [Int]()
        for i in startYear..<nowYear() { years.append(i) }
        return years
    }

    /// 全部月份
    func allMonths()->Array<Int>{
        var months = [Int]()
    ...
    /// 当前年
    func nowYear()->NSInteger{
        let dateformatter = DateFormatter()
        dateformatter.timeZone = TimeZone.current
        dateformatter.calendar = Calendar.current
        dateformatter.dateFormat = "yyyy"
        let str = dateformatter.string(from: self.base)
        return NSInteger(str) ?? 0
    }

    /// 当前月
    func nowMonth()->NSInteger{
        let dateformatter = DateFormatter()
        dateformatter.timeZone = TimeZone.current
        dateformatter.calendar = Calendar.current
    ...
    /// 判断两个日期是否相等
    func equal(_ date:Date)->Bool{
        if self.nowYear() == date.jq.nowYear() && self.nowMonth() == date.jq.nowMonth() && self.nowDay() == date.jq.nowDay(){
            return true
        }
        return false
    }
    ...
    /// 当天周数
    func nowWeekDay()->(weekName:String,index:NSInteger){
        let calendar = Calendar(identifier: .gregorian)
        let a = calendar.component(Calendar.Component.weekday, from: self.base)
        var weekday = "未知"
        var index = 0
        switch a {
            case 1:
    ...
    func nowWeekDay(identifier:Calendar.Identifier)->(weekName:String,index:NSInteger){
    ...
    func formatInfo(_ formatter:String = "MM:DD")->String{
        return ""
    }
    ...
    func weekOfYear()->Int{
        ...
        calendar.firstWeekday = 2
        let weekOfYear = calendar.component(.weekOfYear,from: self.base)
        return weekOfYear
    }
...
public extension Date{
    ...
    static let solarTolunar = [0x04bd8,0x04ae0,0x0a570,0x054d5,0x0d260,0x0d950,0x16554,0x056a0,0x09ad0,0x055d2,
